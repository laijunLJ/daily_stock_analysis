...
```json
{
    ...
    "risk_warning": "风险提示：并明确点出最可能推翻本次结论的那一个关键风险（反证条件）",
...
6. **有据可依**：每条关键判断必须落到具体数据块（技术面/资金流/新闻/财报/基本面）；没有数据支撑的论断要么不写，要么标注“缺乏数据，仅为推测”，严禁编造数字或事件。
7. **讲清把握**：用 `confidence_level` 给出置信度，并严格按标准区分、不要无差别一律给“中”——**高**：趋势、均线、量价、基本面/分析师等多个相互独立的信号同向一致，且你所依据周期的核心数据新鲜完整；**低**：关键信号相互矛盾，或核心数据缺失/拉取失败/过期，或处于方向不明的震荡区；**中**：仅部分信号一致、仍有明显保留时。并在 `risk_warning` 中点出“最可能推翻本结论的关键风险（反证条件）”。
...
- 盘前、非交易日或未知阶段不得伪造今日盘中走势。置信度按“你判断所依据的周期”的核心数据质量来定：日线/波段判断主要看 daily_bars，日内判断主要看 quote/technical——只有当**该周期的核心数据**存在 missing、fetch_failed、stale 或 estimated 时，`confidence_level` 才不得为高；盘前/盘后若日线数据完整且多个信号一致，针对日线/波段结论仍可给“高”。"""
...
```json
{
    ...
    "key_points": "3-5个核心看点，逗号分隔",
    "risk_warning": "风险提示：并明确点出最可能推翻本次结论的那一个关键风险（反证条件）",
    "buy_reason": "操作理由：每条关键判断须指明依据哪类数据（技术面/资金流/新闻/财报/基本面），无数据支撑的论断不得给出",
                ...
                if stream:
                    try:
                        stream_response = call_litellm_with_param_recovery(
                            ...
                            model=model,
                            call_kwargs={**call_kwargs, "stream": True, "stream_options": {"include_usage": True}},
                            model_list=recovery_model_list,
